...
**Rôle :** Algorithme de Matching & Logique Métier Backend  
...
└── matching/                      # Package Python contenant le cœur du module
    ├── __init__.py                # Initialisation du package pour les imports Python
...
1. Format du Mentorat (Score de base - Max 50 points) :
   * Si le mentor propose un format en PRESENTIEL : +50 points
   * Si le mentor propose un format EN_LIGNE ou EN LIGNE : +40 points
2. Bonus de Filière Académique (Max 20 points) :
   * L'algorithme compare la filière de l'apprenant et celle du mentor.
...
3. Poids des Compétences Communes (Max 30 points) :
   * Chaque correspondance exacte (exact match) sur une compétence technique ajoute des points supplémentaires au score global de l'algorithme.
...
{
  "status": "success",
  "message": "Le serveur Flask du Module 4 (Matching) est en ligne et prêt !"
}
...
### Prérequis Systèmes
1. Python 3.x installé globalement sur la machine.
2. Environnement virtuel Python (venv) activé.
3. Serveur de base de données démarré avec la base `ifri_mentorlink`.

### Procédure d'Interconnexion :
1. Nous devons connecter toutes les machines de l'équipe (Frontend et Backend) sur le même point d'accès Wi-Fi.
2. Maintenant, moi je lance le serveur depuis mon terminal avec : python backend_membre4_matching/app_M4.py
3. On note l'adresse IP qui s'affiche dans la console de démarrage (ex: Running on http://192.168.1.103:5000).
4. Les membres du Frontend doivent simplement utiliser cette IP dans leurs configurations pour envoyer les requêtes vers mon serveur.

⚠️ NOTE SUR LA BASE DE DONNÉES LOCALES :
Les identifiants de connexion MySQL présents dans le code de connexion (matching.py) correspondent à ma configuration locale. Si vous récupérez ce code pour exécuter le serveur sur votre propre machine, veillez à modifier la chaîne de connexion avec vos propres identifiants (Utilisateur root et mot de passe local).